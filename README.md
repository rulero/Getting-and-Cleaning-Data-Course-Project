...
2. Extracts only the measurements on the mean and standard deviation for each measurement.
  ...
  * Notice that all the columns that may contain the word *mean* as part of a longer word but doesn't contain it in the form *-mean()* were *not* included, only the variables explicitly associated to the mean() and std() calculations.
...
1. **activity\_subject\_means.txt**.  It contains the data in the tidy data set generated in step 5 of the instructions, it is created with the function `write.table()` using the `row.name=FALSE` parameter, this file is not included in the project but it has been uploaded to Coursera and was allocated in this link https://s3.amazonaws.com/coursera-uploads/user-d0d9b667ac55438824d8619b/975117/asst-3/fec362207a0911e5b0efb1e0630fcdc3.txt.
  ...
  View(data)
  ```
  (Command basics from https://thoughtfulbloke.wordpress.com/2015/09/09/getting-and-cleaning-the-assignment)
  
2. **variables.txt**.  It contains the column names programmatically generated by the script from the original feature names and present in the data set saved in the previous file, not explicitly required by the project and not included here, it is created as an input to make the code book which is included instead.


## The code book

Also included in this project is the file **CodeBook.md** this file contains a detailed description of the tidy data set information in the text file *activity\_subject\_means.txt*, it explains the origin of signal records and the transformations done by the original team that executed the experiments and calculated several variables on it to produce the input files of this project, it explains the criteria used to select the features used in this project and the columns that where rejected, and finally describes the additional transformations done to summarize them.  It also includes a data dictionary describing every column in the file.
